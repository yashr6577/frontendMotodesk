...
function AdminInventoryContent() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await fetch("http://localhost:5000/order/getOrder");
    ...
    const fetchInventory = async () => {
      try {
        const response = await fetch("http://localhost:5000/inventory/"); // Adjust the endpoint as needed
        const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:5000/price/add", {
        method: "POST",
        headers: {
  ...
  const handleAcceptOrder = async (orderId) => {
    try {
      const response = await fetch(`http://localhost:5000/order/order/update/confirmed/${orderId}`, {
        method: "POST",
      });

      if (response.ok) {
        // Fetch updated orders after accepting
        const updatedOrdersResponse = await fetch("http://localhost:5000/order/getOrder");
  ...
  const handleDeliverOrder = async (orderId) => {
    try {
      const response = await fetch(`http://localhost:5000/order/order/update/delivered/${orderId}`, {
        method: "POST",
      });
